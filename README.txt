cl-shell provides a processing model inspired by the Unix shell
pipeline facility. It replaces the shell programming concept of filters
with lambda expressions  and channels. Like the shell pipeline facility,
cl-shell is  a facility for programming with multiple, concurrent threads.

The intention is to emulate  the ease and simplicity of shell
programming whtin the context of a Lisp application and, at the same time,
to provide a model for concurrent programming in the era of multi-core
processors.

Whereas the shell pipeline utility relies on filters and standard I/O, cl-shell
correspondingly uses lambda expressions and channels.

Each stage in the pipeline is a thread. At the heart of cl-shell
is the -> macro, which is used to create a pipeline from a set
of one or arguments.
